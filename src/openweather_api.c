...
bool check_openweather_token(const char* token){
    ...
    sprintf(link, "https://api.openweathermap.org/data/2.5/weather?q=London&appid=%s", token);
...
bool parse_raw_weather_to_city_weather(mem_buff* mb,city_weather* cw){
    ...
    JSON_FIND_HELPER
    struct json_object* result_element = json_object_array_get_idx(weather_obj, 0);
...
bool get_weather_by_city(const char* OW_TOKEN, const char* city, city_weather* cw){
    ...
    sprintf(link_buffer,"https://api.openweathermap.org/data/2.5/weather?q=%s&appid=%s", escaped_city,OW_TOKEN);
